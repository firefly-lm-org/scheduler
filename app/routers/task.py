...
from app.utils.minio_client import get_presigned_download_url, minio_client
...
async def claim_task(
    ...
    # 5. 生成预签名下载 URL（仅当任务实际带包时；模拟任务无包返回 None）
    config = json.loads(claimed_task.config_json or "{}")
    download_url = None
    if claimed_task.task_package_url:
        download_url = await get_presigned_download_url(
            claimed_task.task_package_url,
            expires_sec=3600,
        )
...
# ─────────────────────────────────────
# POST /submit-file  节点提交结果文件（multipart）
# ─────────────────────────────────────
...
async def submit_task_file(
    ...
    """
    节点以 multipart/form-data 上传结果 zip 文件
    服务端：保存 MinIO → 校验 SHA256 → 更新任务状态
    """
    node = await get_active_node(user, db)
